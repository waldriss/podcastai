...
const ProfileDropDown = () => {
  ...
  return (
    <>
       <Input type="file" accept="image/jpeg,image/png,image/webp" className="hidden" ref={imageRef}
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent className="bg-black-3 z-40 mr-3 border-none text-white-1  py-2">
     
        <DropdownMenuLabel className="px-6">My Account</DropdownMenuLabel>
        <DropdownMenuSeparator className="bg-black-2 " />
        <DropdownMenuGroup>
        <DropdownMenuItem onClick={() => imageRef?.current?.click()} className="hover:backdrop-brightness-150 px-6 cursor-pointer" >Upload Profile Image</DropdownMenuItem>
        <DropdownMenuItem onClick={()=>signOut()} className="hover:backdrop-brightness-150 px-6 cursor-pointer"> Log Out</DropdownMenuItem>
        </DropdownMenuGroup>
